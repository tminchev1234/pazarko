...
def _normalize_for_match(name: str) -> str:
    """Strip store-specific prefixes and normalize for cross-store name matching."""
    n = (name or "").lower()
    for w in ("смартфон", "smartphone", "телефон", "мобилен", "mobile phone", "gsm "):
        n = n.replace(w, " ")
    n = re.sub(r"[^a-z0-9а-яА-Я ]+", " ", n)
...
💶 САМО ЕВРО (€). България е в еврозоната — ВСИЧКИ суми в отговора са в евро (€), включително бюджети, спестявания, вноски и примери. НИКОГА не пиши „лв", „лева" или „лв.". Ако потребителят по навик даде бюджет в лева, мълчаливо го преобразувай (лева ÷ 1.9558 = €) и отговори само в €. Формат: „349 €" или „269,00 €".
...
▶ BUDGET_MATCH — категория + бюджет ("лаптоп до 800 €", "телефон около 400€")
...
✅ Спецификации от твоето знание (не от базата) → маркирай: "По спецификация на модела: ..."
   и ЗАДЪЛЖИТЕЛНО добави уговорка да се провери: напр. „(ориентировъчно — потвърди на
   сайта на производителя)". Конкретни числа (W зареждане, чип, MP, mAh, Hz), които НЕ са
   в базата, ВИНАГИ носят тази уговорка. По-добре „не съм сигурен за точната стойност",
   отколкото уверено грешно число.
...
❌ НЕ казвай "най-ниска цена" / "най-евтино" като АБСОЛЮТНА пазарна претенция — следим само
   избрани магазини. Казвай "най-ниска сред магазините, които следим" и при нужда добави
   "другаде може да е по-евтино". Никога не твърди, че няма по-евтино другаде.
...
• deal_score=lowest/good → "най-ниска цена, откакто следим (дъно X € на <дата>) — добър момент"
• deal_score=real → "реална отстъпка — под обичайната цена (~X €)"
• deal_score=suspicious (is_fake_discount) → "внимание, това е привидно намаление — цената е около
  обичайните X € през целия период; преди месец беше <price_30d_ago> €"
• deal_score=high → "близо до собствения пик — по-добре изчакай"
• Ако е поевтиняло/поскъпнало, кажи с числа: "преди ~месец беше <price_30d_ago> €, сега <current> €"
• Когато потребителят пита за конкретен модел — с get_prices сравни магазините и посочи в кой е
  най-евтино в момента и с колко (напр. "в Ozone е с 40 € по-евтино от eMAG").
...
КАЧЕСТВО / РЕВЮТА (комбинирай знанието си с нашите цени):
• За ПОПУЛЯРНИ модели (iPhone, Samsung, MacBook, известни ТВ/лаптопи) ползвай общите си
  познания за качество от ревюта: „M3 е по-бърз чип", „тази камера е по-слаба на тъмно",
  „този панел е по-ярък". Съедини с НАШАТА реална цена: „за +20 € взимаш по-добра батерия —
  тук е най-евтино в Ozone."
• ЧЕСТНО: казвай само каквото знаеш със сигурност. За нишови/малко известни БГ марки
  (STARCREST, CUBOT, Auusda и подобни) НЕ измисляй мнение за качество — кажи „нямам достатъчно
  надеждни ревюта за този модел; съди по спецовете и цената", или предложи по-известна алтернатива.
• Никога не представяй предположение като факт. Ако не си сигурен — кажи го.
...
• Когато сподели съмнителна оферта/сайт или „намерих го за X €" — провери цената срещу реалния пазар.
...
• При въпрос за вноски/лизинг покажи РЕАЛНОТО оскъпяване + ефективен ГПР, не само месечната вноска.
  Пример: „24 вноски = общо 1340 € за телефон, който е 999 в брой → +34% (ГПР ~28%). В брой спестяваш 341 €"
...
• Особено когато по-евтин модел е с лош клас: „Този хладилник е с 40 € по-евтин, но клас E — плаща
  +25 €/година ток. За 5 г. по-скъпият клас A се изплаща и почваш да печелиш."
...
1. Предложи разпределение по категории (напр. "800 € работен setup")
...
3. Категорията е phones / laptops / tvs / tablets И цената е над 300 €
...
ALEX_TOOLS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "(2) категорията е phones/laptops/tvs/tablets и цената е над 300 €, "
    ...
    },
    {
        ...
        "description": (
            ...
            "'намерих го за X €, нормално ли е', или сподели съмнително евтина оферта. "
    ...
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "cash_price":      {"type": "number",  "description": "Цена в брой (€)"},
                "months":          {"type": "integer", "description": "Брой вноски"},
                "monthly_payment": {"type": "number",  "description": "Месечна вноска (€)"},
...
def _price_analysis(rows: list[dict]) -> dict:
    ...
    if len(prices) < 8:
        ...
        forecast = {"action": "wait", "confidence": conf, "target_price": round(p25, 2),
                    ...
                    "basis": "цената пада в момента; обикновено стига до ~%.0f €" % p25}
    elif current >= median * 1.03 and deal_zone >= 0.10:
        forecast = {"action": "wait", "confidence": conf, "target_price": round(p25, 2),
                    "expected_saving_pct": round((current - p25) / current * 100),
                    "basis": "над обичайното; пада до ~%.0f € в ~%d%% от дните" % (p25, round(deal_zone * 100))}
...
def _buy_timing_advice(a: dict) -> str:
    ...
    if s == "lowest":
        ...
        base = f"Близо до най-ниската (дъно {a['low']:.0f} € на {a['lowest_date']}) — разумен момент."
    elif s == "real":
        base = f"Реална отстъпка — под обичайната цена ({a['median']:.0f} €)."
    elif s == "suspicious":
        base = ("Внимание: „намалението“ е привидно — цената е около обичайните "
                f"{a['median']:.0f} € през целия проследен период.")
    elif s == "high":
        base = f"Близо до собствения пик ({a['high']:.0f} €) — по-добре изчакай."
    else:
        base = f"Обичайна цена. Най-ниско е било {a['low']:.0f} € на {a['lowest_date']}."
    f = a.get("forecast") or {}
    act = f.get("action")
...
def _exec_estimate_tradein(args: dict) -> dict:
    ...
    return {
        ...
        "advice": (
            f"При продажба в OLX очаквай {low}–{high} € "
...
def _exec_check_price_sanity(args: dict) -> dict:
    ...
    if claimed < low * 0.6:
        verdict = "scam_likely"
        msg = ("Нереално ниска — %.0f € е %d%% под най-ниската реална пазарна цена (%.0f €). "
               "Класически белег на измама." % (claimed, round((1 - claimed / low) * 100), low))
    elif claimed < low * 0.85:
        verdict = "suspicious"
        msg = "Подозрително ниска — под пазарното дъно (%.0f €). Провери магазина внимателно." % low
    elif claimed <= median * 1.02:
        verdict = "fair"
        msg = "Реалистична цена (пазар: %.0f–%.0f €)." % (low, median)
    else:
        verdict = "above_market"
        msg = "Над средното (%.0f €) — има по-евтино другаде." % median
...
_KWH_PRICE_BGN = 0.30   # битова цена на тока (€/kWh, с мрежови такси)
_ANNUAL_KWH = {
    ...
    for n in names:
        ...
        if sb is not None:
            try:
                ...
                # махни аксесоари (калъфи/кабели) — иначе 'macbook air' връща калъф за 45 €
...
_DEAL_STOP = {"за", "на", "с", "и", "до", "от", "под", "над", "най", "евтин",
              "евтина", "цена", "€", "бг", "нов", "нова", "the", "a", "with"}
...
class DealSubscribeRequest(BaseModel):
    ...
    query:     str = ""
    category:  Optional[str] = None
    max_price: Optional[float] = None   # бюджет: 'известявай ме за X под Y €'
    consent:   bool = False
...
async def deal_check(
    ...
    if avg_30d and history_points >= 3:
        ...
        if old_price > avg_30d * 1.15:
            verdict    = "suspicious"
            confidence = "high"
            reason     = f"Историческата средна цена е {avg_30d:.0f} € — обявената стара цена е {((old_price/avg_30d-1)*100):.0f}% по-висока от реалната пазарна."
        elif current_price <= avg_30d * 1.05:
            verdict    = "real"
            confidence = "high"
            reason     = f"Текущата цена ({current_price:.0f} €) е под или близо до историческата средна ({avg_30d:.0f} €). Намалението изглежда реално."
        else:
            verdict    = "suspicious"
            confidence = "medium"
            reason     = f"Цената e над историческата средна от {avg_30d:.0f} € — намалението може да е частично."
        ...
        if old_price > max_comp * 1.20:
            verdict    = "suspicious"
            confidence = "medium"
            reason     = f"Конкурентите продават на макс. {max_comp:.0f} € — обявената стара цена ({old_price:.0f} €) е {((old_price/max_comp-1)*100):.0f}% по-висока от пазара."
        elif current_price <= avg_comp * 1.05:
            verdict    = "real"
            confidence = "medium"
            reason     = f"Текущата цена ({current_price:.0f} €) съответства на пазарното ниво (средно {avg_comp:.0f} €). Намалението е реално спрямо конкурентите."
        elif current_price <= min_comp * 1.10:
            verdict    = "real"
            confidence = "medium"
            reason     = f"Цената е близо до най-евтиния конкурент ({min_comp:.0f} €)."
        else:
            verdict    = "unknown"
            confidence = "low"
            reason     = f"Конкурентните цени варират ({min_comp:.0f}–{max_comp:.0f} €). Не може да се прецени еднозначно."
