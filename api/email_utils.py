...
def send_deal_alert(
    *,
    ...
    """Известие за силна оферта по интент (какво е търсил потребителят). True при успех."""
    if not smtp_user or not smtp_pass:
        logger.warning("[email] SMTP credentials not configured — skipping deal alert")
        return False

    disc = f"−{discount_pct}%" if discount_pct else "оферта"
    subject_text = f"Alex: {disc} за {product_name[:44]} — {price:.0f} лв."
    ...
    old_html = (
        f'<span style="text-decoration:line-through;color:#9ca3af;font-size:16px;margin-left:8px">{old_price:.0f} лв.</span>'
        if old_price else ""
    )
    badge = (
        f'<span style="background:#dc2626;color:#fff;font-size:13px;font-weight:700;padding:3px 10px;border-radius:6px">−{discount_pct}%</span>'
...
<body style="font-family:Arial,sans-serif;background:#f5f5f5;margin:0;padding:20px">
  <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.1)">
    ...
    <div style="padding:24px">
      <p style="margin:0 0 14px;color:#6366f1;font-size:13px;font-weight:600">🔔 Силна оферта за „{query}"</p>
      {img_html}
      <h2 style="margin:0 0 10px;font-size:16px;color:#1a1a2e">{product_name}</h2>
      <div style="margin-bottom:18px">
        {badge}
        <span style="font-size:28px;font-weight:700;color:#16a34a;margin-left:8px">{price:.0f} лв.</span>
        {old_html}
      </div>
      <p style="margin:0 0 20px;color:#555;font-size:14px">Магазин: <strong>{store}</strong></p>
      <a href="{product_url}" style="display:inline-block;background:#6366f1;color:#fff;text-decoration:none;padding:12px 28px;border-radius:8px;font-weight:600;font-size:15px">Виж в магазина →</a>
    </div>
    <div style="padding:16px 24px;border-top:1px solid #f0f0f0;font-size:12px;color:#9ca3af">
      Получаваш това, защото поиска известия за „{query}" в Alex.<br>
      <a href="{unsubscribe_url}" style="color:#9ca3af">Отпиши се</a>
    </div>
  </div>
</body></html>"""
    ...
    try:
        with smtplib.SMTP(smtp_host, smtp_port, timeout=20) as srv:
            ...
            srv.login(smtp_user, smtp_pass)
            srv.sendmail(smtp_user, [to_email], msg.as_string().encode("utf-8"))
        logger.info("[email] deal alert sent to %s for %r", to_email, query[:40])
        return True
    except Exception as exc:
        logger.error("[email] deal alert to %s failed: %s", to_email, exc)
        return False
